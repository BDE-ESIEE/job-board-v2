...
namespace Macao\UserBundle\Security\Core\User;
...
class FOSUBUserProvider extends BaseFOSUBProvider
{
    ...
    {
        $client_hd = $this->serviceContainer->getParameter('client_hd');
        if ($client_hd != '') {
            if (!array_key_exists('hd', $response->getResponse())) {
                throw new AuthenticationException('Vous devez posséder une adresse edu.esiee.fr');
            }
            else if ($response->getResponse()['hd'] != $client_hd) {
                throw new AuthenticationException('Vous devez posséder une adresse edu.esiee.fr');
            }
        }
